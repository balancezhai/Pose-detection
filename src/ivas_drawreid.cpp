...
static void Drawkeypoints( 
  Point startpint, Point endpoint,
  Mat& lumaImg, Mat& chromaImg, color clr)
{
  ...
  cv::line(lumaImg, startpint, endpoint, Scalar(yScalar),
                                  2 );
  cv::line(chromaImg, startpint/2, endpoint/2, Scalar(uvScalar),
                                  2 );
...
extern "C"
{
  int32_t xlnx_kernel_init (IVASKernel * handle)
  {
    ...
    LOG_MESSAGE (LOG_LEVEL_DEBUG, "xxxxx\n");
     ...
     convert_rgb_to_yuv_clrs(clr, &yScalar, &uvScalar);
           ...
           child_predictions = g_slist_next(child_predictions))
      {
               ...
               classes; classes = g_list_next(classes))
          {
              ...
              if ( classification->class_prob != (float) 888) //(int64_t)child->reserved_2 != -1 && 
              {
              DrawReID( inframe, kpriv,
               ...
               //printf("i = %d; drawing boxes\n", i);
              }
              else{
                  if((int) classification->class_prob == 888){
                      //  for (size_t i = 0; i < pois.size(); ++i) {
                        pois[i].x = child->bbox.x;
                        // std::cout << ((float*)&results)[i * 2] << " " << ((float*)&results)[i * 2
                        // + 1] << std::endl;
                        pois[i].y = child->bbox.y;
                        circle(lumaImg, pois[i], 3, Scalar(yScalar));
                        circle(chromaImg, pois[i]/2, 3, Scalar(uvScalar));
                      // }
                      i++;
                       if(i % 14 == 0)
                       {
                      ...
                      //         circle(lumaImg, pois[i], 3, Scalar(yScalar));
                      //         circle(chromaImg, pois[i], 3, Scalar(uvScalar));
                      //       }
                          //green joints
                          Drawkeypoints(pois[0], pois[1], lumaImg, chromaImg, {0, 255, 0});
                          Drawkeypoints(pois[1], pois[2], lumaImg, chromaImg, {0, 255, 0});

                          Drawkeypoints(pois[3], pois[4], lumaImg, chromaImg, {0, 255, 0});
                          Drawkeypoints(pois[4], pois[5], lumaImg, chromaImg, {0, 255, 0});

                          Drawkeypoints(pois[6], pois[7], lumaImg, chromaImg, {0, 255, 0});
                          Drawkeypoints(pois[7], pois[8], lumaImg, chromaImg, {0, 255, 0});

                          Drawkeypoints(pois[9], pois[10], lumaImg, chromaImg, {0, 255, 0});
                          Drawkeypoints(pois[10], pois[11], lumaImg, chromaImg, {0, 255, 0});

                          Drawkeypoints(pois[12], pois[13], lumaImg, chromaImg, {0, 255, 0});
                          //yellow body
                          Drawkeypoints(pois[13], pois[0], lumaImg, chromaImg, {0, 255, 255});
                          Drawkeypoints(pois[13], pois[3], lumaImg, chromaImg, {0, 255, 255});
                          Drawkeypoints(pois[3], pois[9], lumaImg, chromaImg, {0, 255, 255});
                          Drawkeypoints(pois[0], pois[6], lumaImg, chromaImg, {0, 255, 255});
                          Drawkeypoints(pois[6], pois[9], lumaImg, chromaImg, {0, 255, 255});


                          //printf("pos x %f pos y %f \n", pois[0].x, pois[0].y);
                          //printf("pos x %f pos y %f \n", pois[1].x, pois[1].y);
                          // Drawkeypoints(img, results.right_elbow, results.right_wrist, Scalar(255, 0, 0), 2,
                          //         mark_w, mark_h);
                          // Drawkeypoints(img, results.right_hip, results.right_knee, Scalar(255, 0, 0), 2,
                          //         mark_w, mark_h);
                          // Drawkeypoints(img, results.right_knee, results.right_ankle, Scalar(255, 0, 0), 2,
                          //         mark_w, mark_h);
                          // Drawkeypoints(img, results.left_shoulder, results.left_elbow, Scalar(0, 0, 255), 2,
                          //         mark_w, mark_h);
                          // Drawkeypoints(img, results.left_elbow, results.left_wrist, Scalar(0, 0, 255), 2,
                          //         mark_w, mark_h);
                          // Drawkeypoints(img, results.left_hip, results.left_knee, Scalar(0, 0, 255), 2,
                          //         mark_w, mark_h);
                          // Drawkeypoints(img, results.left_knee, results.left_ankle, Scalar(0, 0, 255), 2,
                          //         mark_w, mark_h);
                          // Drawkeypoints(img, results.head, results.neck, Scalar(0, 255, 255), 2, mark_w,
                          //         mark_h);
                          // Drawkeypoints(img, results.right_shoulder, results.neck, Scalar(0, 255, 255), 2,
                          //         mark_w, mark_h);
                          // Drawkeypoints(img, results.left_shoulder, results.neck, Scalar(0, 255, 255), 2,
                          //         mark_w, mark_h);
                          // Drawkeypoints(img, results.right_shoulder, results.right_hip, Scalar(0, 255, 255),
                          //         2, mark_w, mark_h);
                          // Drawkeypoints(img, results.left_shoulder, results.left_hip, Scalar(0, 255, 255), 2,
                          //         mark_w, mark_h);
                          // Drawkeypoints(img, results.right_hip, results.left_hip, Scalar(0, 255, 255), 2,
                          //         mark_w, mark_h);

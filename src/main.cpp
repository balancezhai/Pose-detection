...
main (int argc, char *argv[])
{
    ...
    if (access("/dev/dri/by-path/platform-a0130000.v_mix-card", F_OK) != 0) //b0000000
    {
    ...
    {
        exec("echo | modetest -M xlnx -D a0130000.v_mix -s 52@40:1920x1080@NV16");//  b0000000  3840x2160
    }
    ...
    for (int i = 0; i < numSrcs; i++)
    {
        ...
        {
            sprintf(pip + strlen(pip),
                    "! kmssink bus-id=a0130000.v_mix plane-id=%d render-rectangle=\"<%d,%d,1920,1080>\" show-preroll-frame=false sync=%s can-scale=false"
                    , 34+validsrc, pos%2*1920, pos/2*1080, std::string(srcenc)=="h265" && std::string(fr)!="30" ? "true" : "false" );//b0000000
        }
        else
        {
            sprintf(pip + strlen(pip),
